...
<script>
import {rechargeWallet, getPackageList} from "../../api/index"
...
export default {
    ...
    mounted(){
        window.getOrderRechargeResult = this.getOrderRechargeResult;
        this.$bridge.registerHandler('getOrderRechargeResult', (params, responseCallback) => {
            this.getOrderRechargeResult(params);
    ...
    methods: {
        ...
        getPackageList() {
           let params = {
               packageType: tools.sessiongetItem("userType"),
           };
        ...
        rechargeWallet() {
           let params = {
                tradeType: this.tradeType, //交易类型 如：0-支付宝、1-微信
                goldMt: this.goldMt,
                silverMt: this.silverMt,
                userId: tools.sessiongetItem("userId"),
                userType: tools.sessiongetItem("userType"),
                totalFee: this.orderCurrency,
                count: this.orderNum,
                packageId: this.packageId
            }
            rechargeWallet(params).then(res => {
                if(res.resp_code == 0) {
                    this.sendRechargeOrderInfo(res.datas);
                } else {
                    Toast(res.resp_msg);
                }
            })
        ...
        sendRechargeOrderInfo(data) {
             let params = {
               rechargeOrderInfo: data,
               tradeType: this.tradeType,   //0 支付宝    1 微信
             };
             tools.mobileDetect() == 1 ? window.webkit.messageHandlers.sendRechargeOrderInfo.postMessage(params) : this.$bridge.callHandler('sendRechargeOrderInfo',params,(res)=>{ responseCallback(params) })
          },
          getOrderRechargeResult(params) {
             this.$toast.clear();
             this.$router.push({
                path: '/walletResult',
                query:{
                    type: JSON.parse(params).tradeState
                }
             })
          },
