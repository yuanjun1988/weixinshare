<template>
  <div class="main"
       @scroll="scrollEvent">
    <img class="activity_close_btn"
         @click="closeActivity"
         src="https://img.bomaleye.com/image/activity_close_btn.png"
         alt="">
    <img class="share"
         @click="shareActivityHome"
         src="http://act.bomaleye.com/planx_share.png"
         alt="">
    <div style="width: 100%;">
      ...
      <div :class="bottom_type"
        ...
        <div style="position: relative;">
          <img src="http://act.bomaleye.com/planx_type_select1.png"
               alt=""
               style="width: 100%;">
    ...
    <div class="promotional_video">
      ...
      <div class="div_play">
        <img class="play"
             src="http://act.bomaleye.com/planx_play.png"
             alt=""
             @click="playVideo"
             v-if="isShow">
      </div>
      <van-swipe class="my-swipe"
        ...
        <van-swipe-item v-for="(image, index) in images"
          ...
          <video class="video_box"
                 ...
                 playsinline
                 preload="auto"
                 poster="https://v.bomaleye.com/bmlyactive0317.mp4?vframe/png/offset/2"
                 :src="image">
          </video>
        </van-swipe-item>
      </van-swipe>
    ...
    <div class="other_activities_entry">
      <div style="position: relative;">
        <img class="img_bg"
             src="http://act.bomaleye.com/planx_home_item_title10.png"
             alt="">
...
export default {
  ...
  created () {
    if (!tools.sessiongetItem("token") || tools.sessiongetItem("token") == null) {
      tools.sessionsetItem("token", tools.GetQueryString('access_token'));
    }
    if (!tools.sessiongetItem("userType") || tools.sessiongetItem("userType") == null) {
      tools.sessionsetItem("userType", tools.GetQueryString('userType'));
    }
    if (!tools.sessiongetItem("userId") || tools.sessiongetItem("userId") == null) {
  ...
  methods: {
    ...
    shareActivityHome () {
      let params = {
        ...
        link: 'https://mp.bomaleye.com/#/planxHome', // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
        imgUrl: 'http://img.bomaleye.com/2021031618145989_680', // 分享图标
      }
      tools.mobileDetect() == 1 ? window.webkit.messageHandlers.shareActivityHome.postMessage(params) : this.$bridge.callHandler('shareActivityHome', params, (res) => { responseCallback(params) })
    },
    playVideo () {
    ...
    // 职场人
    workingPeople () {
      if (tools.sessiongetItem("userType") == 0) {
        Toast('请切换成千里马身份参加活动');
        return;
      }
...
.main {
  height: auto;
  width: 100%;
  background: url(http://act.bomaleye.com/planx_home_bg.png) no-repeat;
  ...
  .promotional_video {
    ...
    background: url(http://act.bomaleye.com/planx_promotional_video_bg1.png)
      no-repeat;
    background-size: 100% 100%;
    .video {
  ...
  .other_activities_entry {
    ...
    .bg1 {
      background: url("http://act.bomaleye.com/planx_home_item_bg1.png")
        no-repeat;
      background-size: 100% 100%;
    }
    .bg2 {
      background: url("http://act.bomaleye.com/planx_home_item_bg2.png")
        no-repeat;
      background-size: 100% 100%;
    }
    .title {
    ...
    .planx_home_item_bg {
      background: url("http://act.bomaleye.com/planx_home_item_bg10.png")
        no-repeat;
      background-size: 100% 100%;
