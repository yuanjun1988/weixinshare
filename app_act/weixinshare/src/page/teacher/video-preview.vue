        ...
        <div class="van_swipe">
            ...
            <van-swipe :show-indicators="false" @change="onChange" vertical :loop="false" ref="swiperBox" :initial-swipe="initial">
                <van-swipe-item v-for="(item, index) in videoList" :key="index" class="product_swiper">
                    ...
                    <!-- 底部作品描述 -->
                    <div class="production_box" v-if="!item.tempVideo || item.tempVideo != 1">
                        ...
                        <div class="vote_box">
                            ...
                            <div class="act-btn">
                                <span class="confirm-btn active" v-if="item.userId != userId && isJoin == 0" @click="handleVote(item)">为TA投票 ></span>
                                <span class="confirm-btn active" v-if="item.userId != userId && isJoin == 0" @click="routerToMakeMoney">分享拉票</span>
                            </div>
                            <!-- <p>每日首次转发可获得三票</p> -->
                        </div>
                    </div>
...
<script>
    ...
    import {getAllActVideoList, handleVote, setShareCount, setActFocus, setActShareStatus, setLikedHandle, getWxSignature, setVoteHandle, getPageCommentList, createComments, deleteComments, setCommentsLike} from "../../api/index"
    export default {
        name: 'home',
        data() {
        ...
        created() {
            ...
            this.videoList = tools.localgetItem('videoList',1);
        },
        mounted() {
            this.$nextTick(()=> {
        ...
        methods: {
            ...
            routerToMakeMoney () {
                let params = {
                    ...
                    link: window.location.href.split('#')[0] + '#/sharePreview?shareVideoId=' + this.videoList[this.current].id + '&userId=' + tools.sessiongetItem('userId'),
           ...
           //展示分享弹窗
            changeShare() {
                let params = {
					...
					link: 'https://mp.bomaleye.com/#/sharePreview?shareType=1' + '&videoType=' + this.videoType + '&shareVideoId=' + this.videoList[this.current].id, // 分享链接
