    ...
    <div class="container_box" id="video_box">
         <div class="content-tab">
             <!-- <div>
                ...
                <div @click="changeTab(2)" :class="videoType == 2 ?'content-tab-item active': 'content-tab-item'">
                    <span>在校生</span>
                    <span class="solid-item"></span>
                </div>
                <div @click="changeTab(3)" :class="videoType == 3 ?'content-tab-item active': 'content-tab-item'">
                    <span>花絮</span>
                    <span class="solid-item"></span>
                </div>
             </div> -->
             <div class="content-tab-item" @click="routerToSharePage">
                 我要报名
             </div>
        </div>
        <div class="van_swipe">
                ...
                <van-swipe-item v-for="(item, index) in videoList" :key="index" class="product_swiper">
                    ...
                    -->
                        <video class="video_box" loop webkit-playsinline="true"  x5-video-player-type="h5-page"
                               x5-video-player-fullscreen="true" playsinline
                    ...
                    <div class="production_box">
                        <div class="production_name">
                            <img class="header_img" v-if="item && item.headportrait" :src="photoFilter(item.headportrait, 74, 74)">
                            <img class="header_img" v-else src="https://img.bomaleye.com/image/coomon_header.png" alt=""/>
                            <span class="userName">{{item.userName}}</span>
                            <div class="vote_num">
                                <img src="http://img.bomaleye.com/20210616150733775_5344" alt="">
                        ...
                        <div class="production_videoTopic">
                            <span>{{item.videoTopic}}</span>
                        </div>
                        <div class="vote_box">
                            <!-- <span :class="!item.savePacket4RedisDto?'':'active'" @click="joinActivity">我要参赛</span> -->
                            <div class="act-btn">
                                <span class="confirm-btn active" @click="handleVote(item)">为TA投票 ></span>
                                <span class="confirm-btn active" @click="routerToMakeMoney">同享赚钱</span>
                            </div>
                            <!-- <p>每日首次转发可获得三票</p> -->
                        </div>
                    </div>
                        ...
                        <div v-if="videoComment.length!=0">
                            <!-- <transition-group appear> -->
                                <li class="comment_li" v-for="(item,index) in videoComment" :key="index">
                                    <div class="comment_author_left" @click="replayUser(item,index,-1)">
                                        <img v-if="item && item.headPortrait" :src="photoFilter(item.headPortrait, 74, 74)">
                                        <img v-else src="https://img.bomaleye.com/image/coomon_header.png" alt=""/>
                                    </div>
                                    <div class="comment_author_right" @click.stop="replayUser(item,index,-1)">
                                        <div class="comment_author_top">
                                            <div class="comment_author_name">
                                                <span>
                                                    <e>{{item.userName}}</e>
                                                    <e v-if="item.userId == videoList[current].userId" class="self_tip_1">作者</e>
                                                </span>
                                                ...
                                                <div class="comment_replay_right">
                                                    <div class="comment_replay_top">
                                                        <div class="comment_replay_name">
                                                            ...
                                                            <span v-if="item.userId == videoList[current].userId" class="self_tip_2">作者</span>
                                                        </div>
        ...
        <van-popup v-model="isParentShow" @closePop='isParentShow = false' class="parsent-dia" :round="true"  @touchmove.prevent :overlay="true" position="bottom" close-icon-position style="background:rgba(255, 255, 255, 0);">
        ...
        <register v-show="coverIsshow2" @handleVote='handleVote' @closeDia='coverIsshow2 = false'></register>
    </div>
</template>

<script>
    var videoProcessInterval;
    ...
    import {getAllActVideoList, getActVideoDetail, setShareCount, handleVote, setActFocus, setActShareStatus, setLikedHandle, getWxSignature, setVoteHandle, getPageCommentList, createComments, deleteComments, setCommentsLike} from "../../api/index"
    export default {
        name: 'home',
        data() {
        ...
        methods: {
            ...
            getActVideoDetail() {
                let params = {
                    videoId: tools.GetQueryString('shareVideoId')
                }
                getActVideoDetail(params).then(res => {
                    if (res.resp_code == 0) {
            ...
            handleVote() {
                ...
                let params = {
                    "playerUserId": this.videoList[0].userId,
                    "playerVideoId": this.videoList[0].id,
                    'sharerUserId': tools.GetQueryString('userId')
                }
                handleVote(params).then(res => {
                    if (res.resp_code == 0) {
                        Toast('投票成功')
                        this.$router.push({
                            path: '/applicationSuccess/' + this.userId + '/' + this.videoList[0].userId + '/' + this.videoList[0].id
                        })
                    } else {
                        Toast(res.resp_msg)
                    }
            ...
            deleteComments(id,index,index2) {
                ...
                deleteComments(params).then(res => {
                    if(res.resp_code == 0) {
                        ...
                        this.videoList[this.current].commentCount --;
                        this.videoList[this.current].subCount --;
                        this.commentsCount --;
                    }
                })
            },
            ...
            changeShare() {
                ...
                getWxSignature(params).then(res => {
                    ...
                    wx.config({
                        ...
                        appId: 'wxc0ab660fe634843a', // 必填，公众号的唯一标识
                    ...
                    var shareData = {
						...
						link: window.location.href.split('#')[0] + '#/sharePreview?shareVideoId=' + this.videoList[0].id + '&userId=' + tools.localgetItem('userId') , // 分享链接
            ...
            changeTab(type) {
                ...
                this.page = 1;
                this.videoList = [];
                this.getAllActVideoList(1);
            },

            //获取活动视频列表
            getAllActVideoList(type) {
                let params = {
                    page: this.page,
                    limit: this.limit,
            ...
            onChange(index) {
                ...
                this.current = index;
                //当播放到倒数第一个，拉取新数据
                if(this.videoList.length - index == 1) {
                    this.page ++;
            ...
            playvideo(event) {
                ...
                this.showShareBox = false;
                //当播放到倒数第一个，拉取新数据
                if(this.videoList.length - this.current == 1) {
                    this.page ++;
                    this.getAllActVideoList();
                }
                video.play();
...
<style scoped lang="scss" rel="stylesheet/scss">
   @function pxTorem($px, $base-font-size: 75px) {
    ...
    .production_box {
        ...
        color: #fff;
        max-width:  pxTorem(582px);
        left: 0;
        box-sizing: border-box;
    }
    .production_videoTopic {
