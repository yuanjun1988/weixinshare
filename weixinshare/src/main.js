...
router.beforeEach((to, from, next) => {
    ...
    // })();
    // console.log(to.path.indexOf('videoVreview'))
    // if (!is_weixin) {
    //       Toast('请在微信app打开链接')
    //       return;
    // }
   ...
   if (to.meta.requireAuth) {
        if (tools.localgetItem('token') == null || !tools.localgetItem('token') || tools.localgetItem('token') == 'undefined' || tools.localgetItem('token') == 'null' || tools.localgetItem('token') == undefined) {
              next({path:'/actLogin'});
            } else {
              next();
            }
