    ...
    <div class="container_box" id="video_box">
        ...
        <div class="van_swipe">
            ...
            <van-swipe :show-indicators="false" @change="onChange" vertical :loop="false" ref="swiperBox" :initial-swipe="initial">
                <van-swipe-item v-for="(item, index) in videoList" :key="index" class="product_swiper">
                    ...
                    <div class="production_box">
                       ...
                       <!-- 下载app按钮 -->
                         <img src="http://img.bomaleye.com/20210302161554231_3598" class="download-app-btn" @click="downloadApp" alt="">
                </van-swipe-item>
            </van-swipe>
            <!--底部操作栏-->
            ...
            <!-- <div class="share_box" :class="showShareBox?'share_active':''">
                ...
                <ul class="share_ul">
                    <li class="share_li pengyouquan_li" @click="shareToFriend">
                        <i class="iconfont icon-pengyouquan pengyouquan"></i>
                    </li>
                    <li class="share_li" @click="shareToTimeline">
...
<script>
    ...
    import {getAllActVideoList, setActFocus, setActShareStatus, setLikedHandle, getWxSignature, setVoteHandle, getPageCommentList, createComments, deleteComments, setCommentsLike, getVideoDetail} from "../../api/index"
    export default {
        name: 'home',
        data() {
        ...
        created() {
            // 分享的数据获取
            if(tools.GetQueryString('shareType')) {
                 this.getVideoDetail()
                 this.videoType = tools.GetQueryString('videoType')
            } 
        },
        mounted() {
        ...
        methods: {
            ...
            getVideoDetail() {
                let params = {
                    id: tools.GetQueryString('shareVideoId'),
                    userType: '1'
                }
                getVideoDetail(params).then(res => {
                    if (res.resp_code == 0) {
                        this.videoList.push(res.datas);
                        tools.localsetItem('videoList', res.datas, 1)
                        this.changeShare();
                    } 
                })
            ...
            setClickStatus() {
                if(!tools.localgetItem("token") || tools.localgetItem("token") == null || tools.localgetItem("token") == undefined) {
                    return false
                } else {
                    return true;
                }
            ...
            changeShare() {
                ...
                getWxSignature(params).then(res => {
                    ...
                    var shareData = {
						...
						link: window.location.href.split('#')[0] + '#/sharePreivew?shareType=1' + '&videoType=' + that.videoType + '&shareVideoId=' + that.videoList[that.current].id, // 分享链接
						imgUrl: 'http://img.bomaleye.com/2021031618145989_680', // 分享图片，使用逗号,隔开
						success: function() {
							that.setActShareStatus()
						}
					};
					wx.ready(function() {
